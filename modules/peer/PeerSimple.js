...
DAD.emitPeers = async function (api, message, peerSet) { // api='/类名/方法名'  向所有邻居发出广播，返回所有结果的数组。可通过 peerSet 参数指定广播对象。
...
DAD.call = async function (api, message, peerSet) { // 随机挑选一个节点发出请求，返回结果。可通过 peerSet 参数指定广播对象。
  peerSet = peerSet || await DAD.getPeerList()
  if (peerSet && peerSet.length > 0) {
