...
POT.api.electWatcher = async function (option) { // 互相转发最优的签名块
  ...
  if (
    ...
    if (option.Block.winnerSignature === wo.Crypto.compareSig((await wo.Chain.getTopBlock()).hash, my.bestPot.signature, option.Block.winnerSignature) // 新收到的签名获胜了。注意，my.bestPot.signature有可能是undefined
      ...
      wo.Peer.emitPeers('electWatcher', { Block: JSON.stringify(option.Block) })
    }
...
POT.forkHandler = async function (option) {
  if ((await wo.Chain.getTopBlock()).height <= Date.time2height() - 2)
    return "高度未达到分叉标准"
  let res = await wo.Peer.call('/Consensus/getRBS', { target: option.Block.packerPubkey })//取第一个元素
  if (!res) {
    mylog.warn("没拿到对方缓存表")
    return null
  }
    ...
    for (let block of blockList) {
      if (block.actionHashList.length !== 0) {
        ...
        for (let action of actionList) {
          wo.Peer.emitPeers('prepare', option) // 将自己区块内的交易广播出去
        }
      }
      await block.dropMe()
    }
