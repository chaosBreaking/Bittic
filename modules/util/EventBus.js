...
function EventBus (obj) {
  if (!new.target) { return new EventBus(obj) }
  this.obj = obj
  this.obj.on('message', async (message) => {
    ...
    if (message && message.event && message.event !== 'call') {
      ...
      if (wo[message.data.who][message.data.act] || wo[message.data.who]['api'][message.data.act]) {
        let callres
        if (!Array.isArray(message.data.param)) {
          callres = message.data.api ? await wo[message.data.who]['api'][message.data.act](message.data.param)
